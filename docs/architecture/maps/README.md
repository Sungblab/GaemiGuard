...
| Broker connection foundation and Toss read-only adapter | `docs/product/broker-connection-and-trading.md`, `docs/toss-invest-openapi.md`, `vendor/tossinvest/openapi-1.0.3.json`, `docs/stages/stage-2-toss-readonly-connector.md` | `packages/shared/src/broker-adapter.ts`, `packages/shared/src/toss-readonly.ts`, `packages/core/src/broker-adapter.ts`, `packages/core/src/toss-readonly-connector.ts`, `packages/core/src/toss-readonly-sync.ts`, `packages/db/src/schema.ts`, `packages/db/src/sqlite.ts`, `apps/api/src/app.ts` | adapter contract tests, connector tests, DB snapshot/manual portfolio tests, API health tests, mutation hard-block tests, `pnpm verify` |
