...
| Product is broker-independent and adapter-based | `docs/product/broker-connection-and-trading.md` | 2+ | broker adapter contract tests, capability metadata tests |
| No-broker mode works before broker login | `docs/product/broker-connection-and-trading.md` | 2+ | first-run/no-broker UI and Commander tests |
...
| Official broker APIs are required | design spec, research, broker direction doc | 2+ | connector tests, no unofficial dependency in production path |
| Toss is first implemented read-only adapter slice | Stage 2 gate | 2 | Toss connector tests, DB snapshot tests |
| Stage 2 broker mutations are blocked | roadmap, Stage 2 gate | 2 | mutation endpoint block tests |
...
| Manual live orders disabled until Stage 6 | roadmap, safety docs | 1-5 | hard block tests |
